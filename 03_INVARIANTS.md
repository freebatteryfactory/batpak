...
| Invariant | Statement |
| --- | --- |
| `INV-ALLOW-IS-DESIGN` | The repo contains ZERO `#[allow(...)]`/`#![allow(...)]`/`#[expect(...)]` attributes. Clippy and rustc findings are FIXED, never silenced; structural-check fails on any allow/expect attribute in tracked source. The ONLY permitted occurrences are the raw-string test fixtures inside the allow-silencer detector itself (crates/core/build_support/shared_checks.rs), which are string data, not attributes. |
...
| `INV-BUILD-FAIL-FAST` | crates/core/build.rs halts the build on invariant violations by returning Err from `fn main() -> Result<(), String>` with a cargo-parseable explanation (the crate-private `fail` helper builds the message). No allow attribute is used. |
...
| `INV-DOCS-CATALOG-VIEW-CURRENT` | 03_INVARIANTS.md is a generated VIEW of traceability/invariants.yaml: the auto-generated catalog block between the BEGIN/END INV-CATALOG markers lists every catalog id and its one-line statement, the authored human prose stays above the block, and the docs-catalog gate folded into structural-check fails on any drift so the published docs can never silently rot away from machine law while the generator regenerates the block deterministically. |
| `INV-DST-CORPUS-CURRENCY` | Every graduated row in `traceability/dst_corpus.yaml` replays through the real Store+SimFs recovery oracle with the stored FNV-1a op-trace digest identity; the structural ledger validates schema and non-emptiness, and this gate proves digest replay currency under `dangerous-test-hooks`. |
...
| `INV-EXAMPLES-OBSERVABLE-OUTPUT` | Examples emit output via `writeln!(std::io::stdout().lock(), ..)` (the explicit Write API), not the println!/print! macros; no print allow is used. |
...
| `INV-FORK-CRASH-ATOMIC` | Store fork survives StoreFs-level seeded faults: after SimFs crash, a fork destination must classify as exactly one of {CommittedPrefix \| RolledBack \| CanonicalRefusal} via the recovery oracle — never an illegal half-fork that open() accepts as valid corrupted state. |
| `INV-FORK-HOSTILE-FS-REFUSAL` | Store fork fails closed under hostile filesystem conditions: a symlink destination leaf is refused with an IO error, a destination canonicalizing to the source data dir is refused as InvalidInput, a destination holding stale store artifacts is positively cleared (DestinationCleared) before the fork so the result is the source state rather than a merge, and an ENOSPC that strikes mid-copy returns a StorageFull error with no openable partial fork published at the destination. |
...
| `INV-GENERATED-WITNESS-PIN` | Derive-generated kind-collision witnesses (the per-payload __batpak_kind_collision_check_<ident> #[test] fn, plus the unconditional inventory registration) detect when two EventPayload types share a (category, type_id) pair: a same-binary collision is a per-payload generated #[test] panic, and a cross-crate collision surfaces through validate_event_payload_registry. The generated identifier is emitted in snake_case (the user's ident is lowered via to_snake_case before being embedded), so the item is a valid snake_case fn name and carries no non_snake_case / lint suppression on generated code. |
...
| `INV-IDEMPOTENCY-AUTHORITY-ATOMIC-COMPACTION` | Compaction plus idempotency-authority persistence is one recoverable commit - the new authority image (and its store.meta expectation) is durably published BEFORE any source segment is deleted and before the pending-compaction marker clears, so every crash or fault boundary yields exactly one legal state, old segments with the old image or compacted segments with the new image (a newer-image-with-rolled-back-segments window is a legal superset admitted by the at-or-past anchor rule); there is no reachable state whose source frames were compacted away while the on-disk image omits their keys, and a failed image publish fails the compaction before destruction rather than being ignored. |
| `INV-IDEMPOTENCY-DURABLE-WINDOW` | A keyed append is deduplicated as a true no-op via a durable AUTHORITY sidecar (index.idemp - magic FBATID, v2 image binding the entries to the store lineage identity and a compound history anchor, crc32fast CRC, atomic write) that survives retention compaction, cold-start, and snapshot independent of event eviction and is restored unconditionally at open (never rebuilt from segment scan); growth is bounded by a window-priority hybrid where the window is inviolable (a key whose recorded global sequence is within keep_sequences of the frontier is never evicted by the soft max_keys cap, which may only trim out-of-window keys), so a retry of a within-window key is always a no-op regardless of load. Damage is never absence (GAUNT-IDEMPOTENCY-AUTHORITY, |
...
| `INV-IMPORT-CRASH-IDEMPOTENT` | Import over a fault-injecting StoreFs survives crash at the durability boundary: abandoning the destination without shutdown and truncating the unsynced tail must leave a reopenable store whose post-crash re-import deduplicates already-applied events via durable import keys while preserving raw payload bytes, content hashes, and a valid per-entity hash chain. |
| `INV-IMPORT-NO-RUNAWAY` | Same-store import terminates at the call-time source visible frontier: it imports exactly the events present when the call began and never paginates into its own freshly-appended output, even when tiny segment rotation interleaves with chunked pagination. |
...
| `INV-INJECTED-CLOCK-OWNS-TIMEKEEPING` | A store driven by a fully injected Clock performs no ambient SystemTime/Instant access anywhere in its lifecycle (open, append, sync, close) — the process-wide monotonic anchor is captured lazily on first monotonic READ of the default SystemClock (or the with_clock_fn adapter), never at clock construction, so the ambient path is unreachable when every reading comes from the injected seam. This is the native-runtime guarantee that a wasm32/workerd embedding (where ambient time traps) can open a store with an injected Clock (#182). |
...
| `INV-MACRO-BOUNDED-CAST` | Integer narrowing inside batpak-macros (under crates/macros) is preceded by an explicit u8::MAX / u16::MAX comparison that returns a syn error before the conversion, and the conversion itself uses a total u8::try_from / u16::try_from(..).expect(..) (expect is permitted in the proc-macro crate) rather than an `as` cast — so there is no unchecked narrowing cast and no clippy::cast_possible_truncation suppression. |
...
| `INV-STORE-LINEAGE-IDENTITY` | Store::identity() returns the store's LINEAGE identity, minted exactly once at the first writable open of a directory and persisted in the store.meta sidecar; it is copied by snapshot and by fork, stable across path moves and reopen, and distinguishes unrelated lineages while deliberately NOT detecting rewind or sibling-fork divergence (external derived state must bind identity PLUS a history anchor). store.meta failure is never absence — corruption and future versions refuse with typed variants, and once a post-migration witness exists (an idempotency sidecar declaring format v2+) a missing store.meta refuses rather than reminting (the never-remint law); read-only opens never mint. |
...
| `INV-UNTRUSTED-MANIFEST-NO-AUTHORITY-ESCALATION` | Under an untrusted segment footer (CRC-failed SDX3, legacy SDX2, or a forged trailer) no SIDX entry-table row can escalate into authority over recovery — a row corroborated against independently verified frame bytes proves only that row's relationship to that frame, never sibling rows, entry_count, order, or the footer boundary (BLAKE3 is public and unkeyed, so stored hashes are copyable). Rows and the footer's claimed frame end are consumed only as suspicion geometry routed through the scan tail posture; the permissive posture recovers the CRC-valid prefix (recording truncation evidence when an untrusted claim reaches past it) and the strict posture refuses every non-empty prefix. Weakening this requires a keyed MAC, digital signature, or independently trusted root over the complete table. |
