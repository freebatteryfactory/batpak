...
`clawbat` is the operation-kit facade.
...
`clawbat` depends on `syncbat`. It does not own dispatch, storage, network
...
Use clawbat to declare what can be run and what references are attached to that
