...
Thin entry shim (BootstrapToolId path). The generator implementation lives in
the sibling ``project`` package; this module loads it by absolute path with its
own submodule search location (no sys.path mutation) and re-exports its full
public surface, preserving the ``--write``/``--check`` CLI contract exactly.
project.py never generates decision text itself.
"""
...
_PACKAGE = Path(__file__).resolve().parent / "project"
...
sys.modules["_project_impl"] = _IMPL
_SPEC.loader.exec_module(_IMPL)

# Re-export the package's entire public surface into module globals so every
# harness `project.<name>` attribute access resolves against this shim.
