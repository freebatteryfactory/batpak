...
def _seedcheck_exe(tree: Path, used_triples: set[str], out_dir: Path) -> tuple[Path | None, str]:
    """Compile the tree's spec rlib and seedcheck binary into out_dir.

    Returns (exe, "") on success, (None, rustc stderr) on a compiler refusal,
    and (None, "unavailable") when no attempted target can link -- linkerless
    environments skip loudly, never silently. The caller owns out_dir's
    lifetime, so a built binary can be aimed at more than one tree."""
    rustc = shutil.which("rustc")
    if not rustc:
        return None, "unavailable"
    exe = out_dir / ("sc" + (".exe" if os.name == "nt" else ""))
    for target in (None, "x86_64-pc-windows-gnu"):
        rlib = out_dir / f"libspec-{target or 'host'}.rlib"
...
def _run_seedcheck(tree: Path, used_triples: set[str]) -> tuple[int, str]:
    """Build the tree's seedcheck and run it against the tree.
    ...
    try:
        exe, err = _seedcheck_exe(tree, used_triples, tmp)
        if exe is None:
            return (-1, "unavailable") if err == "unavailable" else (1, err)
...
def test_seedcheck_executes_its_law(_audit) -> list[str]:
    ...
    def run_seedcheck(tree: Path) -> tuple[int, str]:
        # Build-and-run against the SAME tree (the module-level helper carries
        # the doc): these probes mutate compiled typed tables, so the binary
        # must be built from the mutated tree to test them.
        return _run_seedcheck(tree, used_triples)
