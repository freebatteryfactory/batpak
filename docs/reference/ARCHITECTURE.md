...
**Cold start** uses a four-tier priority: (1) if an `index.fbati` mmap index exists, open it with O(1) cost and let the OS page in entries on demand via lazy page faults — no deserialization, no allocation; (2) if an `index.ckpt` checkpoint file exists and is valid, restore the full index snapshot from it and replay only the tail of segments written after the checkpoint watermark; (3) read SIDX footers from sealed segments in parallel via rayon, then sequentially scan the active segment — this rebuilds the index without full msgpack deserialization; (4) fall back to frame-by-frame segment scanning. Sealed segments are memory-mapped via `memmap2` for zero-copy reads; the active segment uses pread (Unix) or seek+read (Windows).

**Secondary scan index**: The current store keeps the AoS maps as the mandatory base view and can fan out into simultaneous SoA, SoAoS, and AoSoA overlays. Legacy `IndexLayout::AoSoA8/16/64` selectors are still accepted for compatibility, but in the multi-view architecture they all enable the tiled overlay rather than selecting separate standalone scan engines. `ViewConfig` defaults to all overlays enabled (base AoS + SoA + SoAoS + AoSoA64 simultaneously). Each additional overlay costs roughly 8 bytes per event for the `Arc` pointers that link back to the base view; disable individual overlays via `index.views.*` flags when memory is tight.
