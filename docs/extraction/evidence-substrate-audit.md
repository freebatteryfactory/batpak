...
- **Matrix v7.2 (2026-05-08): Pre-Moonwalker closure — `StoreResourceEvidenceReport` ships in core;
...
| Audit assertion runner | `cargo xtask evidence-audit` (static anchors + vocabulary) | `tools/integrity/src/evidence_audit.rs`; extend with deeper AST rules as needed | run before Moonwalker host work when touching evidence bodies | implement in batpak tooling/helper | none | **v1 shipped** — runtime doctrine remains in `tests/evidence_report_family.rs` |
...
| process/sandbox/supervisor evidence | Moonwalker Host planning (`docs/extraction/moonwalker-host-process-boundary-evidence.md`) | N/A in batpak core | keep deployment/runtime orchestration semantics above batpak | implement above batpak | moonwalker-host | composes `PlatformEvidenceSummary`; no store runner primitive |
...
- process/sandbox/supervisor evidence (`docs/extraction/moonwalker-host-process-boundary-evidence.md`)
