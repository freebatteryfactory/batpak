---
category: handoff
provenance: moonwalker
status: non-SOT extraction memo
classification: Handoff only. Does not change Moonwalker behavior, canonical ownership, Pack law, or batpak API.
definition: Self-contained upstream design-spec seed for the batpak owner/agent. Moonwalker references are traceability only; batpak work should be derivable from this file without reading the Moonwalker doc spine.
---

# BATPAK_EXTRACT.md - batpak upstream design-spec seed

This file is intentionally under `archive/`. It is **not** one of Moonwalker's eight canonical root docs and is not source-of-truth for Moonwalker behavior.

Use it as a design seed for batpak work: what should become domain-neutral black-box substrate physics, what already exists in batpak, what APIs/docs/tests a batpak coding agent should draft, and what must never be pushed down because it carries Moonwalker law.

batpak should grow the reusable black-box physics. Moonwalker should shrink to lawful composition. But every candidate below must first prove which kind of proof object it emits -- Event, Receipt, Report, Observation, Envelope, or Ref -- and must not import Pack-authored meaning into the generic mechanism.

**Self-contained rule:** a batpak agent should be able to read this file alone. Moonwalker file paths below are backlinks for the Moonwalker deletion pass, not prerequisites for batpak design. If a row cannot be understood without opening a Moonwalker doc, rewrite the row here rather than asking batpak to learn Moonwalker.

The project owner also owns batpak. That changes the social state but not the architectural state: no external permission is needed to accept a row into batpak, but Moonwalker docs still cannot shrink until the batpak API exists, ships, and is cited as a dependency contract. This file is therefore a design map for batpak and a deletion map for Moonwalker.
...
| Retired | Moonwalker cites shipped batpak APIs and root owner sections have shrunk. | Cite the batpak version / docs and paired Moonwalker shrink PR. |
...
Archive extraction memos may seed external repo work, but Moonwalker agents must not implement behavior from them unless a root MW owner cites the resulting shipped dependency.

## Boundary Covenant

batpak may get bigger. That is not a threat to Moonwalker. The boundary is not "small batpak, large Moonwalker"; the boundary is:

> batpak owns generic runtime mechanics. Moonwalker owns Pack-authored meaning.
...
| batpak-provided mechanism, Moonwalker-owned meaning | batpak provides generic mechanics; Moonwalker composes them into Pack law | sandbox/process evidence primitives composed into Pack-emitted sandbox profiles; lifecycle/readiness evidence composed into `HostDeploymentTarget`; generic schema harness composed into Pack IR and protocol fixture checks |
| Moonwalker-owned semantic law | Depends on Pack-authored agent execution meaning and must not move down | Pack, Pack.lock, Mission, Session, Run, Capability, Criticality, Budget, Topic, Effect, Port, Operation, six-membrane admission meaning, protocol adapter law, host exposure policy, IMAGE, Capsule, human approval, settings authority chain |

The danger is not a more capable batpak. The danger is a generic batpak mechanism being documented as if it owns Moonwalker deployment law. Safe phrasing:

> batpak may record, verify, and in narrow store-owned cases enforce generic process/evidence boundaries. Moonwalker composes those primitives into Pack-emitted sandbox, deployment, and service-supervisor artifacts.

Concrete split:

- batpak may own service-supervisor evidence primitives if they are generic; Moonwalker owns service-supervisor artifact projection for Pack deployments.
- batpak may own sandbox/process boundary primitives if they are generic; Moonwalker owns Pack-emitted sandbox policy.
- batpak may own IPC/event delivery primitives if they are generic; Moonwalker owns substrate-mediated operations that cross those portals under Pack authority.
...
- Moonwalker macros compose those derives into Mission lifecycle, capability checks, six-membrane admission, Pack IR lowering, and Port/protocol projections.
...
That is why the extraction guide is allowed to be larger than a simple TODO list. The goal is not only "move text out of Moonwalker." The goal is to describe the reusable batpak surface clearly enough that a batpak implementation agent can act without learning Moonwalker law.
...
The batpak extraction succeeds only when the batpak repo can answer these questions without reading Moonwalker:
...
If those questions require opening Moonwalker, this seed failed.
...
### Public API Before Moonwalker Shrink

Moonwalker can shrink only after batpak exposes a public API or an explicit docs guarantee.

Private batpak internals are not enough. A private module can change without a consumer-facing migration contract. Moonwalker needs either:
...
| Subscriber frontier | `store::subscription` | lag, loss, consumed frontier, policy observation | PCP, UI, user presence |
...
The next real artifact should live in batpak, not in this Moonwalker archive file. Export the first batch as batpak issues or ADR stubs, then mark this memo `Exported` with issue / ADR ids.

Suggested export batch:

| Export row | Batpak issue / ADR id | Export status | Exported on | Paired Moonwalker shrink target |
|---|---|---|---|---|
| Deterministic receipt-chain evidence report | TODO | not exported | TODO | remove generic receipt walking prose from Moonwalker receipt-audit / macro audit sections after batpak ships |
...
| Subscriber frontier for lossy subscriptions | TODO | not exported | TODO | shrink slow-consumer / lag mechanics to PCP / Highway meaning plus batpak citation after batpak ships |
...
At least two examples must fit the proposed API without Moonwalker vocabulary. If fewer than two fit, do not push the mechanism down.
...
| docs guarantee text | patch-stable once cited by Moonwalker |
| private helper changes | internal unless exposed in docs |

Moonwalker cannot cite an unstable private detail. If batpak wants freedom to change a candidate, mark it experimental and keep Moonwalker shrink blocked.
...
These names are suggested generic batpak vocabulary. They are not Moonwalker primitive claims.
...
| `Receipt` | Evidence that an append, denial, projection, read, lifecycle transition, or verification action happened. | Moonwalker admission approval. |
...
| `DenialReceipt` | Evidence that a requested append/action was rejected by a generic gate or invariant. | Moonwalker denial taxonomy. |
| `Gate` | Generic predicate / admission point that returns pass, deny, defer, or diagnostic evidence. | Moonwalker's six-membrane law. |
| `Pipeline` | Ordered composition of generic gates or stages with proof of traversal. | Moonwalker Pack Pipeline or admission pipeline. |
| `Projection` | Deterministic view derived from durable facts. | Protocol, UI, skill, SBOM, or Pack projection meaning. |
| `Frontier` | A named point in write/apply/visible/durable/replay progress. | A user-visible Moonwalker success state. |
| `VisibilityFence` | A guarantee that selected work is visible before a continuation proceeds. | Pack-level recovery or approval. |
| `Cursor` | Replay / subscription position with restart discipline. | PCP context position or Mission Control state. |
...
| `DEFER` | Plausible but too easy to import Moonwalker law today. | Leave out until multiple non-agent users need it. |
| `DO_NOT_IMPORT_PACK_LAW` | Agent / Pack meaning. | Keep out of batpak. |

## RFC Card Template

Every batpak extraction RFC should fit this card. If it cannot, the candidate is probably too Moonwalker-shaped.
...
Minimum tests:
Non-goals:
Moonwalker shrink trigger:
```

The most important line is `Generic users`. A good answer is "financial ledger, robotics recorder, compliance event store, local-first sync engine." A bad answer is "Moonwalker needs this for Missions."

## High-Priority RFC Cards

These cards are the first extraction batch. They are intentionally written without requiring Moonwalker docs.
...
It proves durable shape and fixture bytes. It does not classify fields as authority-bearing, privacy-sensitive, context, freshness, provenance, or side-effectful. Those semantic classes belong to Moonwalker / protocol-registry layers above batpak.
...
Moonwalker shrink trigger: once this exists, Moonwalker should stop explaining generic receipt walking and keep only its receipt fields and denial/authority extensions.
...
Moonwalker shrink trigger: once this exists, Moonwalker can cite batpak for generic schema/golden-byte testing and retain only which Moonwalker types and protocol fixtures are pinned.
...
Non-goals: PCP, user presence, Mission Control, protocol subscription semantics.

Moonwalker shrink trigger: once this exists, Moonwalker can stop carrying slow-consumer mechanics and keep only PCP/Highway meaning.
...
- Moonwalker projection: emit ecosystem-shaped artifacts such as MCP, A2A, AG-UI, OTel, SBOM, docs, and audit surfaces.
- protocol projection: render byte-for-byte protocol fixtures or descriptors.

`ProjectionRunner` may only own the first altitude. It must not emit protocol descriptors, skill formats, OTel spans, SBOMs, or Moonwalker docs.
...
Moonwalker shrink trigger: once this is public, Moonwalker docs keep projection meanings but delete generic runner tutorials.
...
Moonwalker shrink trigger: Moonwalker can cite this for resource observation and keep only policy consequences.
...
Moonwalker shrink trigger: Moonwalker can stop hand-explaining generic signed envelopes and keep Pack-specific identity law.
...
Moonwalker shrink trigger: Moonwalker can cite this for generic registry attestation and keep protocol/Pack semantic mapping above.
...
- protocol/application registry: MCP/A2A/ACP/PCP semantics, semantic field classes, drop policy, normalization profile, adapter mapping, conformance levels, watcher bots, automated PR generation.
...
Non-goals: PCP, retrieval ranking, token budgeting, authorization semantics.

Moonwalker shrink trigger: Moonwalker can cite this for generic context/read evidence and keep context semantics above.
...
Moonwalker shrink trigger: Moonwalker may compose generic lifecycle evidence while keeping its own lifecycle law.
...
Class: `DEFER` / external adjunct candidate until two non-Moonwalker, non-budget consumers need the same generic shape.
...
Moonwalker shrink trigger: Moonwalker can compose this for budgets/effects if useful while keeping policy above.
...
Moonwalker shrink trigger: Moonwalker can delete generic platform/process mechanics once batpak exposes them, while retaining deployment law.
...
| deployment/supervisor law | application / Moonwalker Host | generated service unit, sandbox profile emission, capability-to-syscall mapping, bind mounts, auth, network exposure, rollout/update rules |
...
| `SubscriberFrontier` + `LagPolicy` | `READY_RFC` | Generic subscriber lag, consumed frontier, lag age, loss marker, and back-pressure policy over lossy subscriptions. | PCP, Mission Control, protocol subscription law. |
...
| `ReadWalkReport` | `CANDIDATE` | Deterministic read evidence with as-of, regions, cursors, dropped rows, limits, proof refs; optional persisted event above it. | PCP, ranking, token budget, authorization. |
...
| `ReservationLedger` | `DEFER` | Durable reserve/commit/refund/expire/orphan mechanics after non-Moonwalker pressure proves the common shape. | Capability supply, Budget tier, Criticality. |
...
| Generic staged-pipeline proof token | `DEFER` | Possible affine stage-token utility for ordered pipelines. | Moonwalker-style named admission membranes. |
...
|---|---|---|
| typed event/receipt derives | `EventPayload`, `EventSourced`, `MultiEventReactor`; public derives in `src/lib.rs`; ADR-0010 | Already real. Moonwalker should compose, not wrap. |
| typed append/query paths | `append_typed`, `submit_typed`, `try_submit_typed`, `BatchAppendItem::typed`, `by_fact_typed`, `Transition::from_payload`; ADR-0010 | Already real. Moonwalker macros should emit these directly, never raw byte-level event paths. |
...
| frontier states | `FrontierView`, `WatermarkSnapshot`, `HlcPoint`, `WatermarkKind` in `src/store/stats.rs`; ADR-0014 | Already real. Moonwalker should cite these for received/written/durable/visible/applied/emitted posture. |
...
| HLC / lifecycle ordering | `HlcPoint`; `SYSTEM_OPEN_COMPLETED` / `SYSTEM_CLOSE_COMPLETED`; reopen monotonicity in store lifecycle | Already real. Moonwalker should cite lifecycle evidence instead of explaining restart physics. |
| durability gates and waits | `DurabilityGate`; `AppendOptions::with_gate`; `Store::wait_for_durable` / `wait_for_applied` / `wait_for_visible`; ADR-0016 | Already real. Moonwalker should only name which Pack declaration selects which watermark. |
| batch atomicity / fenced batch submit | `append_batch`, `append_batch_with_options`, `BatchAppendItem`, batch BEGIN/COMMIT markers, `VisibilityFence::submit_batch` | Already real. Do not add Moonwalker batch mechanics unless they carry Pack compensation/Effect meaning. |
| outbox staging | `Store::outbox`, `Outbox::stage`, `Outbox::flush`, `Outbox::submit_flush`; guide section "Outbox" | Already real for batch-shaped internal commits. Does not own Moonwalker Effect/Port/provider delivery semantics. |
| append idempotency | `AppendOptions::with_idempotency`, `StoreError::IdempotencyRequired`, group-commit idempotency invariant | Already real for append/batch dedupe. Moonwalker still owns wire-level idempotency scope and replay-response semantics. |
| publish-before-reply visibility | `publish_then_broadcast_unfenced` establishes visibility before append returns | Already real. Moonwalker read-your-writes doctrine should cite this instead of re-teaching publish ordering. |
| coordinate-region containment | `coord_in_region(coord, region)` segment-boundary predicate | Already real or batpak-bound by Moonwalker G2. Moonwalker must not implement prefix matching locally. |
| at-least-once witnesses | `AtLeastOnce`, `CheckpointId`, `ObservedOnce` in `src/store/delivery/observation.rs`; ADR-0017 | Already real. Moonwalker cursor-reactor should use directly. |
...
| canonical encoding | `pub use crate::encoding as canonical`; docs say stronger canonical-bytes contract is still phased in | Exists but needs strengthened version contract/tests before Moonwalker can delete caveat prose. |
...
This is the conceptual target for batpak. Names are suggested batpak names, not Moonwalker commitments. Build against the `Boundary` column; do not chase Moonwalker-specific anchors.
...
| A | `SchemaSnapshot` harness | Add only if existing derives/schema tests cannot cover fixture needs. | Knows canonical bytes; not MCP/A2A/PCP semantics. |
...
| B | `SubscriberFrontier` / `LagPolicy` | Add public slow-consumer frontier over `Subscription` / `subscribe_lossy`. | Knows lag/loss; not PCP/Mission Control authority. |
...
- batpak docs can explain receipt continuity without Moonwalker
- canonical encoding contract is line-citable
- schema/golden fixture drift can fail a batpak test
- Moonwalker can cite public batpak APIs for chain walking and schema snapshots
...
3. Keep `ReservationLedger` deferred unless non-Moonwalker pressure proves the shared shape.
...
The batpak repo should gain docs that stand alone from Moonwalker.
...
It should not mention Moonwalker except as one consumer in an examples list, if at all.
...
## Negative Prompt Filter For Moonwalker Shaving

Use this section when reading Moonwalker root docs. If a paragraph primarily describes one of these generic mechanisms, it probably does not belong in canonical Moonwalker prose except as a short dependency placeholder.

| If Moonwalker prose explains... | Replace with placeholder for... | Moonwalker keeps... |
|---|---|---|
...
| slow subscriber lag/loss/backpressure mechanics | `SubscriberFrontier` / `LagPolicy` | PCP/Highway consumer meaning and user-visible consequences |
...
| read-as-of / walked regions / dropped rows / proof refs | `ReadWalkReport` | PCP meaning, retrieval ranking, token budget, auth |
...
| deterministic audit result rows | deferred `AuditAssertionRunner` / helper crate | named Moonwalker audit queries and filter grammar |
...
mechanism to batpak and leave a placeholder in Moonwalker.
```
...
or human approval, keep it in Moonwalker.
```
...
## Moonwalker Shrink Protocol

After a batpak API ships, Moonwalker should shrink in three steps:

1. Replace generic mechanism tutorial with a one-paragraph dependency citation.
2. Keep the Moonwalker semantic law.
3. Keep the receipt/admission/projection fields that are Moonwalker-specific.
...
```text
Moonwalker explains how to walk receipt parent links, check hashes,
...
```text
Moonwalker uses batpak::ReceiptChainWalker for generic receipt-chain
verification. Moonwalker-specific law is limited to which receipt fields
...
- Moonwalker pins a version containing it
- tests prove the cited guarantee

Moonwalker shrink PRs cannot merge on intent alone. A shrink PR must prove:

- the Moonwalker workspace pins a batpak version that contains the cited public API
- batpak docs or ADRs are line-citable for the guarantee
- Moonwalker tests or compile checks consume the public API where applicable
- old Moonwalker generic-mechanism prose is replaced with a dependency citation plus Moonwalker-owned meaning

## Paired Deletion Tickets

Every high-priority batpak export row should have a paired Moonwalker deletion candidate. The paired deletion does not execute until the batpak release gate above is satisfied.

| Batpak export row | Moonwalker deletion candidate | Keep in Moonwalker |
|---|---|---|
| `ReceiptChainWalker` / receipt-chain evidence report | Delete generic hash / parent / fork / cycle walking tutorial prose. | receipt envelope fields, authority-bearing meaning, Moonwalker denial mapping |
...
| `SubscriberFrontier` / `LagPolicy` | Delete generic slow-consumer lag / loss / backpressure mechanics. | PCP / Highway consumer meaning and user-visible consequences |
...
```text
Added Moonwalker support for Missions.
```

batpak should say what batpak provides, not what Moonwalker composes.

## Rubber-Duck Backlog

Six doc-specific read-throughs on 2026-05-07 found these additional generic candidates. Treat this table as self-contained batpak backlog; the Moonwalker terms in the boundary column are warning labels, not prerequisites.
...
| Multi-link `ReceiptChainWalker` | Verify named receipt links, not only linear hash chain: parent exists, content hash matches, no cycles, no fork without declared branch. | Link names are generic; Moonwalker owns causation-vs-authority meaning. |
...
| Versioned fact lifecycle | Append-only fact lifecycle with `live`, `faded`, `superseded_by`, and preserved existence marker. | No Topic, PII, KeyedBinding, GDPR, or Moonwalker extension keys. |
...
| Read-walk report | Read/context evidence: coordinates walked, cursor positions, as-of point, dropped entries, budget/limit counters, proof handles. | No PCP, retrieval ranking, token-budget, or Capability semantics. |
| Audit result envelope | DEFER/helper crate: generic evidence row for lint/property/audit result: test name, outcome, evidence hash, observed/expected fields, and source range. | No Moonwalker twenty-pass harness names. |
...
| Forensic query envelope | Read-only evidence rows with deterministic ordering, canonical JSONL/dCBOR output, and no storage-iteration-order dependence. | No Moonwalker audit filter grammar or named queries. |
...
| Fault-injection / audit snapshot hooks | Append fault injection points and coherent watermark/frontier snapshot hooks for tests. | No Moonwalker red-team fixture names. |
| Thread/process lifecycle evidence | Generic named worker/process started, ready, stopped, crashed, restarted, drain-began, drain-completed evidence. | No `mw-*` thread names, Highway, PCP, SystemMission, or Port law. |
...
These are forbidden as public semantic types, enums, modules, traits, or feature names. Literal strings may appear in non-goals, fixtures, registry-row examples, or tests proving no vocabulary leak. A generic registry row with stable id `"pcp"` is fine; `batpak::PcpContextBundle` or `batpak::ProtocolSpecName::Mcp` is not.

If a batpak design wants one of those public names, it is accidentally becoming Moonwalker.

## Moonwalker Shrink Ledger

Moonwalker docs stay large until these rows land in batpak. After a row ships, shrink the named Moonwalker owner sections to composition law plus a dependency citation.

| If batpak ships... | Moonwalker sections to shrink | Keep in Moonwalker |
|---|---|---|
| `ReceiptChainWalker` | `MW_FEATURES.md` receipt envelope audit prose; `MW_MACRO.md` receipt-stream walking/audit mechanism prose | receipt envelope fields, Moonwalker denial/authority/cause extensions |
...
| `FrontierView` / `VisibilityFence` | repeated frontier state explanations across `MW_FEATURES.md`, `MW_RUNTIME.md`, `MW_HOST.md` readiness/recovery prose | which Moonwalker action advances which frontier and what receipt is emitted |
...
| `SubscriberFrontier` / `LagPolicy` | `MW_RUNTIME.md` push-down candidate row; `MW_MACRO.md` PCP subscriber-lag mechanics | PCP meaning, Highway-vs-PCP distinction, Mission Control consumer behavior |
...
> Once batpak owns a generic mechanism, Moonwalker should not keep a parallel tutorial for that mechanism. Moonwalker should keep only the Pack-authored meaning, the owner section, and the citation to the batpak API.

That is how the docs get smaller.

## Classification Meanings

- `READY_RFC` - clean non-agent substrate candidate; write a batpak RFC with minimal Moonwalker vocabulary.
- `SPLIT` - generic observation/accounting can move down; Moonwalker keeps policy consequences.
...
- `DEFER` - plausible abstraction, high risk of importing Moonwalker law.
- `DO_NOT_IMPORT_PACK_LAW` - leave in Moonwalker unless a genuinely generic primitive emerges.
...
- Do not use this file as a canonical Moonwalker owner section.
...
A row graduates from handoff memo to Moonwalker dependency only when all are true:

1. batpak accepts and ships the domain-neutral API.
2. Moonwalker pins the batpak version that contains it.
3. `MW_RUNTIME.md` Part 2 cites the accepted batpak API as part of the boundary contract.
4. Existing Moonwalker owner sections shrink to cite the boundary instead of re-specifying the generic mechanism.
