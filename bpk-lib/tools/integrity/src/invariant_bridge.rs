...
pub(crate) fn check(repo_root: &Path, tracked_files: &[PathBuf]) -> Result<()> {
    ...
    let citation_waivers = load_waivers(&trace_dir.join("invariant_citation_waivers.yaml"))?;
    let ledger_waivers = load_waivers(&trace_dir.join("ledger_prose_waivers.yaml"))?;
    ...
    check_catalog_test_coverage(repo_root, &invariants, &artifact_map);
    Ok(())
}
...
fn is_test_module_path(path: &str) -> bool {
    path.starts_with("crates/core/tests/")
        && path.ends_with(".rs")
...
fn load_waivers(path: &Path) -> Result<CitationWaivers> {
    if !path.exists() {
