...
fn collect_root_layout_findings(repo_root: &Path, out: &mut Vec<Finding>) {
    ...
    if !tracked_plans.is_empty() {
        out.push(Finding {
            ...
            fix: "Remove tracked plan files or promote durable content into cookbook/200_*.md files or traceability/agent_surface.yaml.",
    ...
    if local_plan_files > 0 {
        out.push(Finding {
            ...
            why: format!(".cursor/plans contains {local_plan_files} local file(s); keep it ignored or promote durable content into root docs or traceability/"),
            fix: "Leave it if the tool requires it, otherwise remove it from tracking after preserving durable decisions.",
            example: "cookbook/200_*.md or traceability/agent_surface.yaml are durable owners; .cursor/plans is not.",
            warning_only: true,
        });
    }
}
        ...
        for stale in [retired_path("adr"), retired_path("extraction")] {
            if content.contains(&stale) {
                out.push(Finding {
                    ...
                    fix: "Update the reference to a factory root doc, bpk-lib/ADR path, or current owner path; do not recreate retired extraction docs by accident.",
                    example: "bpk-lib/ADR/100_ADR_0019_CANONICAL_ENCODING_CONTRACT.md",
                    warning_only: false,
                });
            }
        }
