//! Behaviour tests for `#[derive(Error)]`.
//!
//! The central guarantee is byte-identical rendering + identical `source()`
//! wiring versus a hand-rolled impl, so several cases assert the derived type
//! against a hand-written reference with the same shape.
