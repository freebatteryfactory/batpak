...
Create a private `bpk-lib/crates/core/src/store/platform/` backend room for target-sensitive
...
- Machine-contact helpers have one internal home under `bpk-lib/crates/core/src/store/platform/`.
...
- `cargo xtask platform ...` owns operator profile probe/verify/bless/audit
  workflows. `bpk-lib/crates/core/build.rs` may validate an explicitly configured profile via
...
- Structural checks prevent direct target-sensitive store calls from leaking
  outside `bpk-lib/crates/core/src/store/platform/`.
...
- No live hardware probing in `bpk-lib/crates/core/build.rs`.
- No profile signing.
