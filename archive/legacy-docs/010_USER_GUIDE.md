...
[`020_TECHNICAL_REFERENCE.md`](020_TECHNICAL_REFERENCE.md) for architecture, topology, tuning, and
invariants.
...
- `bpk-lib/crates/core/benches/replay_lanes.rs` is the current witness surface for the replay-lane
  tradeoff and currently shows `RawMsgpackInput` ahead on the 1k-event
  counter-shaped workload in this tree
- `bpk-lib/crates/core/examples/event_sourced_counter.rs` is the ergonomic default template
- `bpk-lib/crates/core/examples/raw_projection_counter.rs` is the performance-lane template
