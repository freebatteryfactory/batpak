...
| `INV-HOSTBAT-STRUCTURAL-SCHEMA` | Client-visible hostbat schemas carry a structural SchemaShape folded into the hostbat.schema.v2 encoding and H_interface (hostbat.interface.v3); SchemaRegistry::validate performs structural checks when a shape is present; client-visible refs without shape fail host build closed; and DiagnosticRustType never affects schema or interface identity. |
