...
use batpak::reservation::{
    ...
    RESERVATION_STATE_REFUNDED, RESERVATION_STATE_RESERVED,
};
...
fn reservation_cause_sorting_digest_stable() {
    ...
    assert_eq!(
        reservation_transition_log_digest(&normalize_reservation_transition_list(&[t1.clone()]))
            .expect("d1"),
        reservation_transition_log_digest(&normalize_reservation_transition_list(&[t2]))
            .expect("d2")
    );
    let b1 = reservation_transition_bytes(&t1).expect("b1");
    t1.cause_refs.reverse();
    let b2 = reservation_transition_bytes(&normalize_reservation_transition(&t1)).expect("b2");
    assert_eq!(b1, b2);
}

#[test]
fn reservation_subject_key_sort_normalizes() {
    let s = subject(0, &[3u8, 1, 2]);
    let n = normalize_reservation_subject_ref(&s);
    assert_eq!(n.key_bytes, vec![1, 2, 3u8]);
}
...
fn reservation_report_findings_order_independent_hash() {
    ...
    assert_eq!(h0, h1);
    assert_eq!(rep.schema_version, RESERVATION_LEDGER_REPORT_SCHEMA_VERSION);
